...
describe('Sign user up', () => {
  it('Sign up in user', () => {
    ...
    cy.get('[data-testid="sign-up-email"]').type(username);
    cy.get('[data-testid="sign-up-email"]').should('have.value', username);
    cy.get('[data-testid="sign-up-firstName"]').type('test');
    cy.get('[data-testid="sign-up-lastName"]').type('user');
    cy.get('[data-testid="sign-up-password"]').type(password);
    cy.get('[data-testid="sign-up-password"]').should('have.value', password);
    cy.get('[data-testid="sign-up-role"]').select('Decision Maker');
    cy.get('[data-testid="sign-up-org"]').select('HRT - Hellenic Rescue Team');
    cy.get('[data-testid="sign-up-agreeTermsConditions"]').check();
