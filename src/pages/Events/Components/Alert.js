...
const Alert = ({ card, alertId, setSelectedAlert, setFavorite, t }) => {
  ...
  return (
    <Card
      ...
      className={
        'alerts-card mb-2 ' + (card.id === alertId ? 'alert-card-active' : '')
      }
    >
            ...
            <Row className="mt-2">
              <Col>
                <small className="text-muted card-desc date no-wrap">
                  {t('Start', { ns: 'common' })}: {formatDate(card.start_date)}{' '}
                  - {t('End', { ns: 'common' })}:{' '}
                  {card.end_date
                    ? formatDate(card.end_date, 'YYYY-MM-DD')
                    : '-'}
                </small>
              </Col>
              <Col md={2}>
                <CardText>
                  <span className="float-end alert-source-text me-2">
                    {card.alerts.map(alert => alert.title).join(',  ')}
                  </span>
                </CardText>
              </Col>
            </Row>
