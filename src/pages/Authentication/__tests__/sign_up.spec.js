...
describe('Sign Up Component', () => {
  ...
  describe('Form testing', () => {
    ...
    test('Fields should be empty initially', () => {
      ...
      expect(screen.getByTestId('sign-up-password-toggle')).toHaveClass('fa-eye')
      expect(screen.getByTestId('sign-up-agreeTermsConditions')).not.toBeChecked();
    })
    ...
    test('Click Agree to terms and conditions to should set remember me to checked', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('sign-up-agreeTermsConditions')).toBeChecked();
      }) 
    })
  ...
  describe('sign up action', () => {
    beforeAll(() => {
      mock = axiosMock;
      mock.onPost(`${baseURL}${endpoints.authentication.signUp}`).reply( 200, USERS);
    })
    beforeEach(() => {
      renderApp(store);
    })
    it('signs up user correctly', async () => {
      //arrange
      act(() => {
        fireEvent.change(screen.getByTestId('sign-up-email'), {target: {value: 'test@gmail.com'}})
        fireEvent.change(screen.getByTestId('sign-up-firstName'),{target: {value: 'john'}});
        fireEvent.change(screen.getByTestId('sign-up-lastName'),{target: {value: 'doe'}});
        userEvent.selectOptions(screen.getByTestId('sign-up-role'), 'e38000a4-e121-427c-94f0-0330741a5a16');
        userEvent.selectOptions(screen.getByTestId('sign-up-org'), '92604bfd-856a-468e-93d1-0f146650805e');
        fireEvent.change(screen.getByTestId('sign-up-password'),{target: {value: 'HelloWorld12'}});
        fireEvent.click(screen.getByTestId('sign-up-agreeTermsConditions'))
      })
      //act
      act(() => {
