...
const ingredientsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.SIGN_UP_OAUTH2_SUCCESS:
      return signUpOauth2Success(state, action);
    case actionTypes.SIGN_UP_OAUTH2_FAIL:
      return signUpOauth2Fail(state, action);
    case actionTypes.SIGN_UP_OAUTH2_REFRESH_SUCCESS:
      return refresTokenSuccess(state, action);
    case actionTypes.SIGN_UP_OAUTH2_REFRESH_FAIL:
      return refresTokenFail(state, action);
    case actionTypes.FORGOT_PASSWORD_SUCCESS:
      return reqResetPswSuccess(state, action);
    case actionTypes.FORGOT_PASSWORD_FAIL:
      return reqResetPswFail(state, action);
...
const signInSuccess = (state, action) => {
  ...
  const updatedState = {
    user: action.payload,
    tokenExpiresIn: action.payload.oauth2
      ? action.payload.oauth2.expires_in
      : null,
    isLoggedIn: true,
    error: false,
  };
