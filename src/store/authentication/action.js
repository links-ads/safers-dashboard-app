...
export const signUpOauth2 = userInfo => async dispatch => {
  const response = await api.post(endpoints.authentication.oAuth2SignUp, {
    ...userInfo,
  });
...
export const signUp = userInfo => async dispatch => {
  const response = await api.post(endpoints.authentication.signUp, {
    ...userInfo,
  });
  if (api.isSuccessResp(response.status)) {
...
export const reqResetPsw = email => async dispatch => {
  const response = await api.post(
    ...
    email,
  ); // To Do: change to post when API ready
  if (response.status === 200)
    return dispatch(reqResetPswSuccess(response.data));
  else return dispatch(reqResetPswFail(response.error));
};
...
export const resetPsw = data => async dispatch => {
  ...
  if (response.status === 200) return dispatch(resetPswSuccess(response.data));
